import { deleteBook, getBookById, like, likeById, myLikeById } from "../api/data.js";
...
export async function detailsPage(ctx) {
    ...
    const [book, likes, hasLike] = await Promise.all([
        ...
        likeById(ctx.params.id),
        userData ? myLikeById(ctx.params.id, userData.id) : 0
    ])
    ...
    async function onDelete() {
        const choice = confirm(`Are you sure you want to delete ${book.title} FOREVER?`);
